<template>
  <div class="github">
    ...
    <button v-on:click="loaded = !loaded">{{ loaded ? "Unload" : "Load" }}</button>
    <Naproche v-if="loaded" :args="['--fof=on', '--prover=spass', filename]" :fetchUser="fetchUser" />
  </div>
</template>
...
export default {
  ...
  computed: {
     fetchUser() {
        return fetchUserGithub(this.username, this.repository, this.branch).fetchUser;
     }
  },
  data() {
    return {
